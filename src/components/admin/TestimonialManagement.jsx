...
const TestimonialManagement = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (editingId) {
      updateTestimonial.mutate({ id: editingId, ...payload }, { onSuccess: () => { setEditingId(null); setFormData({ name: '', role: '', content: '', avatar_url: '' }); } })
    } else {
      addTestimonial.mutate(payload, { onSuccess: () => setFormData({ name: '', role: '', content: '', avatar_url: '' }) })
    }
  }

  const handleEdit = (testimonial) => {
    setEditingId(testimonial.id)
    setFormData({ name: testimonial.name, role: testimonial.role, content: testimonial.content, avatar_url: testimonial.avatar_url })
  }
    ...
    <div className="space-y-6">
      <form onSubmit={handleSubmit} className="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg space-y-3">
        ...
        <div className="flex gap-2">
          ...
          {editingId && <button type="button" onClick={() => { setEditingId(null); setFormData({ name: '', role: '', content: '', avatar_url: '' }); }} className="bg-gray-500 text-white px-4 py-2 rounded">Cancel</button>}
        </div>
      </form>
